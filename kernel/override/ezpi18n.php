...
class ezpI18n
{
    ...
    protected static $allowedContexts = null;
    ...
    */
     ...
     * @return string
     */
    public static function tr( $context, $source, $comment = null, $arguments = null, $plain = false )
    {
        if ( self::isEnabled() )
        {
            $locale = eZINI::instance('site.ini')->variable('RegionalSettings', 'ContentObjectLocale');
            $translatedText = self::translateText( $context, $source, $comment, $arguments );

            $allowedContexts = self::getAllowedContexts();
            if(count($allowedContexts) > 0) {
            ...
            if($plain === true) {
                ...
                return "<span contenteditable class='hwi18n' data-locale='$locale' data-context='$context' data-source='" . base64_encode($source) . "'>$translatedText</span>";
            }
        }
    ...
    protected static function getAllowedContexts() {
        if(self::$allowedContexts != null) {
            return self::$allowedContexts;
        }
        $ini = eZINI::instance('hwi18n.ini');
        self::$allowedContexts = $ini->variable( 'Translations', 'AllowedContexts' );
        return self::$allowedContexts;
